...
clustdthreshold = config['clusters_depth_analysis'][-1]['cluster_fit_std_threshold']
...
'''
##############################################################################                                                                                   
#CLUSTER SEARCH FOR ACCURATE DARK CURRENT MEASURE#############################                                                                                   
##############################################################################
...
if reportCalibrationDarkcurrent and nskips!=1:
    ...
    skipper_avg_exposed_L = skipper_avg_cal_L
    parametersDCfit_U, offset_U, nbins_plot_U = m_calibrationdc.calibrationDC(skipper_avg_exposed_U, stdmanyskip_U[-1], reverse, debug=False)
    calibrationconstant_U = parametersDCfit_U[0][5]; calibratedsigma_U = stdmanyskip_U[-1]/calibrationconstant_U
    skipper_avg_cal_U = reversign*(skipper_avg0_U - offset_U)/calibrationconstant_U
    skipper_avg_exposed_U = skipper_avg_cal_U
    ...
    with doc.create(Section('Pixel Charge Distributions and Noise')):
        ...
        if nskips!=1:
            try:
                ...
                if calibrationconstant_L <= 1 or calibrationconstant_L > 2*calibrationguess:
                    ...
                    print('WARNING a: inaccurate calibration constant. Using guess value. Will not report on calibration')
            except:
                calibrationconstant_L = calibrationguess
                reportCalibrationDarkcurrent_L = False
                print('WARNING b: calibration constant not defined for ADU/e- noise conversion. Using guess value')
            ...
            if reportCalibrationDarkcurrent_L:
                ...
                axs[1].hist(avg_image_0ravel_L, 200, range=(-3*calibrationconstant_L,7*calibrationconstant_L), density = False, histtype='step', linewidth=2, log = True, color='teal', label = 'avg img pixel charge distribution')
        ...
        if nskips!=1:
            ...
            if reportCalibrationDarkcurrent_U:
                ...
                axs[1].hist(avg_image_0ravel_U, 200, range=(-3*calibrationconstant_U,7*calibrationconstant_U), density = False, histtype='step', linewidth=2, log = True, color='teal', label = 'avg img pixel charge distribution')
...
if reportCalibrationDarkcurrent and nskips!=1:
    ...
    if not applymask: skipper_avg_cal_L = np.ma.masked_array(skipper_avg_exposed_L, mask=None)
    ...
    skipperavgcalibratedravel_L = skipper_avg_exposed_L.compressed()
    ...
    try: plt.title('$I_{CF}~=~$' + str(round(dcpar_L[0],6)) + r'$\pm$' + str(round_sig_2(dcparunc_L[0])) + ' $e^-$pix$^{-1}$, sigma ' + str(round(dcpar_L[3],3))  + r'$\pm$' + str(round(dcparunc_L[3],3)) +' e-')
    except: plt.title('$I_{CF}~=~$' + str(round(dcpar_L[0],6)) + r'$\pm$' + str(dcparunc_L[0]) + ' $e^-$pix$^{-1}$, sigma ' + str(round(dcpar_L[3],3)) + r'$\pm$' + str(round(dcparunc_L[3],3)) +' e-')
    ...
    try: plt.title('$I_{CF}~=~$' + str(round(dcpar_U[0],6)) + r'$\pm$' + str(round_sig_2(dcparunc_U[0])) + ' $e^-$pix$^{-1}$, sigma '+ str(round(dcpar_U[3],3)) + r'$\pm$' + str(round(dcparunc_U[3],3)) + ' e-')
    except: plt.title('$I_{CF}~=~$' + str(round(dcpar_U[0],6)) + r'$\pm$' + str(dcparunc_U[0]) + ' $e^-$pix$^{-1}$, sigma '+ str(round(dcpar_U[3],3)) + r'$\pm$' +str(round(dcparunc_U[3],3)) +' e-')
